    ...
    const plugin = (Plugin, Api) => {
      ...
      return class extends Plugin {
        ...
        async onStart() {
          ...
          // check for pgpDataStore file
          if (!fs.existsSync(path.join(BdApi.Plugins.folder, "pgpDataStore.json"))) {
            ...
            // save keys to pgpDataStore
            pgpDataStore.privateKey = privateKey
            pgpDataStore.publicKey = publicKey
            pgpDataStore.revocationCertificate = revocationCertificate




            fs.writeFileSync(path.join(BdApi.Plugins.folder, "pgpDataStore.json"), JSON.stringify(pgpDataStore));
          } else {
            pgpDataStore = JSON.parse(fs.readFileSync(path.join(BdApi.Plugins.folder, "pgpDataStore.json")));
          }
          ...
          BdApi.Patcher.before(
            ...
            async (_, data) => {
              ...
              if (encryptMessages) {
                ...
                message = this.encryptMessage(message).then((encryptedMessage) => {
                  Logger.info(`Encrypted Message: ${encryptedMessage}`)
                  data[1].content = encryptedMessage
                  return data;
                })
              }

              data[1].content = message;
              return data;
            }
          )
        }
        ...
        encryptMessage = async (message) => {
          ...
          const { privateKey, publicKey } = pgpDataStore
          let encryptedMessage = await openpgp.encrypt({
